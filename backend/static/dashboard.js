  ...
  imageInput.addEventListener("change", async (e) => {
    ...
    try {
      ...
      if (data.detections && data.detections.length > 0) {
        ...
        addToFeed(`🖼️ ${data.detections.length} detection(s) found in image.`);
      } else {
        addToFeed("🖼️ No detections found in image.");
      }
    } catch (err) {
      console.error("Detection failed:", err);
      addToFeed("⚠️ Image detection failed (backend unreachable).");
    }
  });
}
...
async function detectLiveFrame() {
  ...
  try {
    ...
    console.warn("Live detect error (backend offline):", e);
  }
...
async function updateSummary() {
  try {
    ...
    document.getElementById("withMaskCount").textContent = s.with_mask ?? 0;
    document.getElementById("noMaskCount").textContent = s.no_mask ?? 0;
    document.getElementById("incorrectCount").textContent = s.incorrect ?? 0;

    const riskValue = document.getElementById("riskScoreValue");
    const riskLabel = document.getElementById("riskLabel");
    const score = s.risk_score ?? (s.no_mask * 5 + s.incorrect * 3);

    riskValue.textContent = Math.min(score, 100).toFixed(0);
...
async function startPolling() {
  try {
    ...
    if (res.ok) {
      ...
      console.log("✅ Backend reachable — polling started.");
    } else {
      console.warn("⚠️ Backend not reachable — polling disabled.");
    }
  } catch {
    console.warn("⚠️ No backend detected — skipping polling.");
  }
}
...
function updateDetectionChart(summary) {
  ...
  detectionChart.data.datasets[0].data.push(summary.with_mask ?? 0);
  detectionChart.data.datasets[1].data.push(summary.no_mask ?? 0);
  detectionChart.data.datasets[2].data.push(summary.incorrect ?? 0);
